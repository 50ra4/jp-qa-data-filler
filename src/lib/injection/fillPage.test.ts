...
describe('fillPage', () => {
  ...
  test.each([
    ...
    ['<input type="number" name="email">', 'UNSUPPORTED_CONTROL'],
    ['<input type="date" name="email">', 'UNSUPPORTED_CONTROL'],
    ['<input type="time" name="email">', 'UNSUPPORTED_CONTROL'],
    ['<input type="week" name="email">', 'UNSUPPORTED_CONTROL'],
    ['<input type="month" name="email">', 'UNSUPPORTED_CONTROL'],
    ['<input type="range" name="email">', 'UNSUPPORTED_CONTROL'],
    ['<input type="color" name="email">', 'UNSUPPORTED_CONTROL'],
  ...
  test('ブラウザが値を受理しない場合はfilledへ数えずeventも送らない', () => {
    setBody('<label>メールアドレス<input></label>');
    const input = document.querySelector<HTMLInputElement>('input');
    const inputEvent = vi.fn();
    input?.addEventListener('input', inputEvent);
    vi.spyOn(HTMLInputElement.prototype, 'value', 'set').mockImplementation(
      function () {
        // Simulate a browser control rejecting an assigned value.
      },
    );
    ...
    expect(result.filled).toHaveLength(0);
    expect(result.skipped).toContainEqual(
      expect.objectContaining({ reason: 'VALUE_REJECTED' }),
    );
    expect(inputEvent).not.toHaveBeenCalled();
  });

  test('layout boxがない項目を実環境では非表示として扱う', () => {
    setBody('<input autocomplete="email">');
    vi.mocked(Element.prototype.getClientRects).mockReturnValue({
      length: 0,
    } as DOMRectList);
  ...
  test('unit test用optionはlayout判定だけを無効化する', () => {
    setBody('<input autocomplete="email">');
    vi.mocked(Element.prototype.getClientRects).mockReturnValue({
      length: 0,
    } as DOMRectList);
  ...
  test('結果配列を上限までに制限し、省略件数を返す', () => {
    setBody(
      Array.from(
        { length: 55 },
        (_, index) => `<input autocomplete="email" aria-label="mail-${index}">`,
      ).join(''),
    );
