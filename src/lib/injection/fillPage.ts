  ...
  while (roots.length > 0) {
    ...
    for (const control of controls) {
      ...
      if (
        ...
        result.skipped.push({ descriptor, reason: 'READONLY' });
        continue;
      }
      if (isHidden(control)) {
        result.skipped.push({ descriptor, reason: 'HIDDEN' });
        continue;
      }
      if (
        control instanceof HTMLInputElement &&
        ...
        result.skipped.push({ descriptor, reason: 'UNSUPPORTED_CONTROL' });
        continue;
      }
      ...
      if ('ambiguous' in classified) {
        result.skipped.push({ descriptor, reason: 'AMBIGUOUS' });
        continue;
      }
      if (
      ...
      try {
        if (control instanceof HTMLSelectElement) {
          ...
          findValueSetter(control)(matches[0].value);
          ...
          if (shouldTruncate) {
            result.warnings.push(
              `${descriptor}: value truncated to maxLength ${maxLength}.`,
            );
          }
          ...
          if (control.value !== value) {
            result.warnings.push(
              `${descriptor}: browser adjusted the assigned value.`,
            );
          }
        }
        result.filled.push({
        ...
        result.skipped.push({ descriptor, reason: 'WRITE_FAILED' });
      }
    }
  }
