...
- 入力・skip理由別・判定不能の正確な件数と、field記述・判定確度・warning・error・省略件数を含む上限付き監査詳細
...
5. popupの正確な件数と上限付き監査詳細を確認してからフォームを確認します。送信が必要なテストでは、適切な環境か確認して手動送信します。

`valid`は文字列形式上の妥当性だけを示します。電話値はseedに応じて変わる`000-0000-XXXX`で、日本の電話番号に似た形ですが、`000` prefixは構造上ルーティング不能なためprefixを検査するvalidatorでは拒否される場合があります。`boundary`はseedに応じて変わる、hyphenなしのより長い全角数字を使い、`invalid`は短い値を使います。電話番号、郵便番号、住所の割当・配達可能性・実在性は保証しません。
