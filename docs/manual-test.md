...
5. Verify recognized name, Kana, email, phone, postal, address, and organization fields are filled.
6. Verify the popup remains open and reports filled, skipped, unmatched, and warning counts.
7. Repeat with the same seed and confirm the generated values are identical.
...
- Attach a `submit` listener and confirm its count remains zero.
...
Closed Shadow DOM, iframes, contenteditable, custom controls, ambiguous fields, and site-specific selector conventions are intentionally unsupported.
